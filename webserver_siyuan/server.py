...
def building_detail(name,address,zipcode):
  ...
  commands_surr = f"""select type_s, count(*) from surroundings
                      where (zipcode,address_surround) in (select s.zipcode_s,s.address_surround from buildings b,building_surround s where s.zipcode='{zipcode}' and s.address='{address}')
                      group by type_s
                    """
